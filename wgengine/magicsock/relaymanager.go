...
func (r *relayManager) ensureDiscoInfoFor(work *relayHandshakeWork) {
	...
	if di.di == nil {
		di.di = &discoInfo{
			...
			sharedKey:  work.wlb.ep.c.discoPrivate.Load().Shared(work.se.ServerDisco),
		}
	}
}
...
func (r *relayManager) allocateAllServersRunLoop(wlb endpointWithLastBest) {
	...
	discoKeys := key.NewSortedPairOfDiscoPublic(wlb.ep.c.discoPublic.Load(), remoteDisco.key)
	for _, v := range r.serversByNodeKey {
